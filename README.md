...
>>> Relational databases is the database software itself (the actual database) 
>>> SQL is a language which is used to query the Relational Database
>>> So the relational database is system or database, and SQL is the language used to interact with it.


1. Why do tables need a `primary key`?
>>> Tables need a primary key because it is a way to uniquely identify each entry in the database (ensures no duplicates).
